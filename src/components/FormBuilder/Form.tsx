...
const getWrappedComponentWithForwardRef = _.memoize((Comp: any) =>
...
function convertMeta(field: any) {
  const widget = getWidget(field.widget);
  const item: any = _.find(
    Object.values(widgetMap),
    (entry: any) => entry.widget === widget && entry.metaConvertor,
  );
  if (item) {
...
class FormBuilder extends Component<any, any> {
  ...
  renderField = (field: any) => {
    ...
    const label = field.tooltip ? (
      <span>
        {field.label}
        <Tooltip title={field.tooltip}> </Tooltip>
      </span>
    ) : (
      field.label
    );
    ...
    let formItemLayout =
      ...
      (field.label ? _.get(meta, 'formItemLayout') || [8, 16] : null);
    ...
    const isFieldViewMode =
      this.props.viewMode || meta.viewMode || field.viewMode || field.readOnly;
    ...
    const formItemProps = {
      ...
      ...field.formItemProps,
      className: `${
        this.props.viewMode || meta.viewMode ? 'ant-form-item-view-mode' : ''
    ...
    if (isFieldViewMode) {
      let viewEle = null;
      const formValues = this.props.form
        ? this.props.form.getFieldsValue()
        : {};
      let viewValue = _.has(formValues, field.key)
        ? _.get(formValues, field.key)
        : initialValue;
      if (field.renderView) {
        viewEle = field.renderView(viewValue, this.props.form);
      } else if (field.viewWidget) {
        const ViewWidget =
          typeof field.viewWidget === 'string'
            ? getWidget(field.viewWidget)
            : field.viewWidget;
        viewEle = (
          <ViewWidget
            value={viewValue}
      ...
      if (!viewEle) {
        if (typeof viewValue === 'boolean')
          viewEle = _.capitalize(String(viewValue));
  ...
  renderLayout(elements: any, fields: any) {
